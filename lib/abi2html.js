...
var AbiHtml = function(abiString) {
    ...
    var abi
    if (typeof abiString === 'string') abi = JSON.parse(abiString)
    else if (typeof abiString === 'object') abi = abiString
    else return null
...
Web3Event.prototype.makeFieldFilter = function(callback) {
    var evmEvent = this.ev
    ...
    if (evmEvent.inputs.length > 0) {
        ...
        div.appendChild(this.makeEventFilterOptions(callback))
            // div.appendChild(h3)
            // div.appendChild(fs)
...
Web3Event.prototype.makeEventFilterOptions = function(callback) {
    ...
    // labAddress.htmlFor = 'eventFilterAddress'
    // labAddress.innerHTML = 'Address:'
    // eventOptions.appendChild(labAddress)

    // var inAddress = document.createElement('input')
    // inAddress.type = 'input'
    // inAddress.value = ''
    // inAddress.id = 'eventFilterAddress'
    // eventOptions.appendChild(inAddress)
