...
AbiHtml.prototype.constructor = function(abiString) {
    ...
    for (var i = 0; i < abi.length; i++) {
        ...
        // console.log(item.name, item, this)
    }
}

AbiHtml.prototype.GetEventsRendered = function(options, filterFields, cb) {
    var that = this
...
AbiHtml.prototype.GetAllEvents = function(options, filterFields, cb) {
    // this has no contect of an AbiItem
    var that = this
    this.contract.at(options.address).allEvents({
        fromBlock: 'earliest',
        toBlock: 'latest'
...
// no callers
AbiHtml.prototype.GetEvent = function(name) {
