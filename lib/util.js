...
AppStatus.prototype.displayPending = function() {
  var el = document.getElementById('pending-tx')
  el.innerHTML = ''
  ...
  for (var index in this.pendingTransactions) {
    ...
    var link = 'tx/' + txhash
    var msg = '<li><a href="#' + link + '">' + foo + '</a></li>'
    el.innerHTML += msg
  }
}

// AppStatus.prototype.displayStatus = function() {
//   var el = document.getElementById('notification-anchor')
//   if (el.length > 0){
//     var block = web3.eth.getBlock('pending')
  
//     anchorRoot.innerHTML  = 'Pending block #' + block.number + '<br>'
//     anchorRoot.innerHTML += 'Gas limit: ' + block.gasLimit + '<hr>'
//     anchorRoot.innerHTML += '<ul id="pending-tx"></ul>'
//     anchorRoot.classList.add('dashed')
