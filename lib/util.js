...
AppStatus.prototype.getCurDoc = function() {
  var i = this.selectedDocIndex
...
AppStatus.prototype.renderCodeView = function() {
  ...
  var ah = this.loadedDocs[this.selectedDocIndex]
  if (ah) {
    var sc = document.getElementById('source-code')
    if (ah.source) sc.value = ah.source
...
AppStatus.prototype.renderActions = function(ah) {
  if (!ah) ah = this.getCurDoc()
  if (!ah) return

  var callCallback = function(web3Function) {
    var foo = function() {
      try {
  ...
  ah.GetFunctionsSorted().forEach(function(f) {
    ...
    var transactCallback = function(web3Function) {
      ...
      if (!connectInstance()) {
        ...
        var kv = []
        for (var i = 0; i < f.func.inputs.length; i++) {
          var field = f.func.inputs[i]
          var val = document.getElementById(field.htmlId).value
          kv.push(val)
        }

        var data = f.contract.at(document.getElementById('option-address').value)[f.name].getData(kv)
        foo({
          to: document.getElementById('option-address').value,
          data: null,
          gas: 300000,
    ...
    article.appendChild(f.makeFieldForm(callCallback, transactCallback))
...
AppStatus.prototype.renderSearch = function(ah) {
  if (!ah) ah = this.getCurDoc()
  if (!ah) return
...
AppStatus.prototype.routeApp = function() {
  ...
  if (hash.length > 0 && parts.length > 0)
    switch (parts[0]) {
      case 'event':
        // this should link to prettified form
        // to do this it needs to figure out the right type from the ABI
        break
      case 'log':
        if (parts.length > 3) {
